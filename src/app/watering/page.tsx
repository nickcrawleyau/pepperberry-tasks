...
import { fetchWateringData } from '@/lib/hydrawise';
import WateringDisplay from '@/components/watering/WateringDisplay';
...
export default async function WateringPage() {
  ...
  if (!session) redirect('/');
  if (session.role !== 'admin' && !session.allowedSections?.includes('watering')) redirect('/dashboard');
  ...
  try {
    watering = await fetchWateringData();
  } catch {
    error = true;
  }
        ...
        <div className="max-w-2xl mx-auto px-5 py-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-lg font-medium text-fw-text truncate">Watering</h1>
              {watering && (
                <p className="text-xs text-fw-text/50">
                  Hydrawise &middot; Updated {new Date(watering.fetchedAt).toLocaleTimeString('en-AU', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'Australia/Sydney' })}
                </p>
              )}
            </div>
          </div>
      ...
      <main className="max-w-2xl mx-auto px-5 py-6">
        {error || !watering ? (
          <div className="bg-fw-surface rounded-xl border border-fw-surface p-8 text-center">
            ...
            <p className="text-sm font-medium text-fw-text/80 mb-1">Watering data unavailable</p>
            <p className="text-xs text-fw-text/50">Check your Hydrawise connection and try again.</p>
          </div>
        ) : (
          <WateringDisplay data={watering} isAdmin={session.role === 'admin'} />
        )}
      </main>
    </div>
  );
